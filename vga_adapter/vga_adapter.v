 ...
 *
 * November 13, 2015 - Modifying the code to support 320x240 and 640x480. The new chip has enough memory for
 *		   the full VGA resolution, so I am bumping it up. (Bill Bateman)
 */
...
module vga_adapter(
	 ...
	 * 
	 * BILL- Changing this to be "320x240" or "640x480." The new chip has more than enough memory.
	 */
	 ...
	 * a given resolution.
	 */
	input [((RESOLUTION == "320x240") ? (8) : (9)):0] x; //BILL - 11/13/15 Changed to from (8) : (7)
	input [((RESOLUTION == "320x240") ? (7) : (8)):0] y; //BILL - 11/13/15 Changed to from (7) : (6)
	...
	output doneFrame;
	//will be 1 when we have just finished drawing a frame to the screen
	
	//input [((MONOCHROME == "TRUE") ? (0) : (BITS_PER_COLOUR_CHANNEL*3-1)):0] colour;
	wire[((MONOCHROME == "TRUE") ? (0) : (BITS_PER_COLOUR_CHANNEL*3-1)):0] messing_with_colour;
	//assuming 3-bit colour channels
	assign messing_with_colour[2] = colour[2];
	assign messing_with_colour[1] = colour[0];
	assign messing_with_colour[0] = colour[1];
	
	assign messing_with_colour[5] = colour[5];
	assign messing_with_colour[4] = colour[3];
	assign messing_with_colour[3] = colour[4];
	
	assign messing_with_colour[8] = colour[8];
	assign messing_with_colour[7] = colour[6];
	assign messing_with_colour[6] = colour[7];
	...
	/* Pixel colour read by the VGA controller for frame 1 */
	wire [((MONOCHROME == "TRUE") ? (0) : (BITS_PER_COLOUR_CHANNEL*3-1)):0] to_ctrl_colour2;
	/* Pixel colour read by the VGA controller for frame 2 */
	
	wire [((RESOLUTION == "320x240") ? (16) : (18)):0] user_to_video_memory_addr; //BILL- changed from 14 to 18
	/* This bus specifies the address in memory the user must write
	 * data to in order for the pixel intended to appear at location (X,Y) to be displayed
	 * at the correct location on the screen.
	 */
	
	wire [((RESOLUTION == "320x240") ? (16) : (18)):0] controller_to_video_memory_addr; //BILL- changed from 14 to 18
	...
	reg writeToFrame1 = 1'b0;
	//if 1, then the user is writing to frame 1 (frame 2 is being read by VGA)
	//if 0, then the user is writing to frame 2 (frame 1 is being read by VGA)
	...
	altsyncram	VideoMemory (
				...
				.data_a (messing_with_colour), // data in
	...
	defparam
		...
		VideoMemory.WIDTHAD_A = ((RESOLUTION == "320x240") ? (17) : (19)), //BILL- changed from 15 to 19
		VideoMemory.NUMWORDS_A = ((RESOLUTION == "320x240") ? (76800) : (307200)), //BILL- changed from 19200 to 307200
		VideoMemory.WIDTHAD_B = ((RESOLUTION == "320x240") ? (17) : (19)), //BILL- changed from 15 to 19
		VideoMemory.NUMWORDS_B = ((RESOLUTION == "320x240") ? (76800) : (307200)), //BILL- changed from 19200 to 307200
		...
		/* Create video memory for frame 2 */
	...
	defparam
		...
		VideoMemory2.INTENDED_DEVICE_FAMILY = "Cyclone V",
		VideoMemory2.OPERATION_MODE = "DUAL_PORT",
		VideoMemory2.WIDTHAD_A = ((RESOLUTION == "320x240") ? (17) : (19)), //BILL- changed from 15 to 19
		VideoMemory2.NUMWORDS_A = ((RESOLUTION == "320x240") ? (76800) : (307200)), //BILL- changed from 19200 to 307200
		VideoMemory2.WIDTHAD_B = ((RESOLUTION == "320x240") ? (17) : (19)), //BILL- changed from 15 to 19
		VideoMemory2.NUMWORDS_B = ((RESOLUTION == "320x240") ? (76800) : (307200)), //BILL- changed from 19200 to 307200
	...
	vga_controller controller(
			.vga_clock(clock_25),
			.resetn(resetn),
			.pixel_colour(writeToFrame1 ? to_ctrl_colour2 : to_ctrl_colour1),
			//if writeToFrame1 == 1, then frame2 is being read by VGA
			//else, frame1 is being read by VGA
		...
		always@(posedge doneFrame)
			writeToFrame1 <= ~writeToFrame1;
			//switch frames
endmodule
